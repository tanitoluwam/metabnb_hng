  ...
  return (
    <nav className="d-md-flex justify-content-between align-items-center mt-5  container mx-auto mb-5">
      <div className="d-flex justify-content-between">
        ...
        <button onClick={() => setOpen(!open)} className="d-md-none bg-white border-0">
          {open ? <IconClose /> : <HamburgerIcon />}
        </button>
      </div>

      <ul className="d-md-flex align-items-center absolute links_contaner">
        {Links.map((link) => (
